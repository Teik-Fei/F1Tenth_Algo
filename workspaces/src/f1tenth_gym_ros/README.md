...
A containerized ROS 2 bridge for the F1TENTH gym environment. Runs the simulator in ROS 2 Foxy with RViz visualization and supports custom autonomous driving algorithms.
...
- (Optional, NVIDIA GPU only) nvidia-docker2: https://docs.nvidia.com/datacenter/cloud-native/container-toolkit/install-guide.html
- (Optional, NVIDIA GPU only) rocker: https://github.com/osrf/rocker
...
### Option A: With an NVIDIA GPU (rocker + X11 forwarding)
...
Start the container with display forwarding:

```bash
rocker --nvidia --x11 --volume .:/sim_ws/src/f1tenth_gym_ros -- f1tenth_gym_ros
```

### Option B: Without an NVIDIA GPU (noVNC in browser)

Start both the simulator container and the noVNC display container:

```bash
docker-compose up
```

Open a bash session inside the simulator container:
...
Click Connect to see the display.

---

## Launching the Simulator

Inside the container, source the workspace and launch the simulator:
...
An RViz window will open showing the car on the map.

Use `tmux` (included in the container) to open additional terminal panes without leaving the container:
...
## Running the Gap Finder Algorithm

In a second terminal (or tmux pane), with the simulator already running:

```bash
source /opt/ros/foxy/setup.bash
source /sim_ws/install/setup.bash
ros2 run f1tenth_gym_ros gap_finder
```

The node subscribes to `/ego_racecar/scan` and publishes drive commands to `/drive`.

---

## Changing the Map

1. Download your map files (`.png` and `.yaml`) into `maps/`:

```bash
# Example: Nuerburgring
curl -L https://raw.githubusercontent.com/f1tenth/f1tenth_racetracks/main/Nuerburgring/Nuerburgring_map.png \
     -o maps/Nuerburgring_map.png
curl -L https://raw.githubusercontent.com/f1tenth/f1tenth_racetracks/main/Nuerburgring/Nuerburgring_map.yaml \
     -o maps/Nuerburgring_map.yaml
```

2. Edit `config/sim.yaml` and update `map_path` to point to the new map (without file extension):

```yaml
map_path: '/sim_ws/src/f1tenth_gym_ros/maps/Nuerburgring_map'
map_img_ext: '.png'
```
...
All simulation parameters are in `config/sim.yaml`:

| Parameter | Description |
|---|---|
| `map_path` | Full path to map file (no extension) |
| `map_img_ext` | Map image extension (`.png` or `.pgm`) |
| `num_agent` | Number of agents: `1` or `2` |
| `scan_beams` | Number of LiDAR beams |
| `scan_fov` | LiDAR field of view in radians |
